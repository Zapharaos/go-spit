...
func TestCSV_processValue(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "Empty slice with separator",
			value:    []interface{}{},
			format:   "",
			listSep:  "|",
			expected: "",
		...
		},
		{
			...
			value:    time.Date(2023, 1, 15, 0, 0, 0, 0, time.UTC),
			format:   "2006-01-02",
			listSep:  "",
			expected: "2023-01-15",
			wantErr:  false,
		...
		{
			name:     "Date without format",
			value:    time.Date(2023, 1, 15, 10, 30, 0, 0, time.UTC),
			format:   "",
			listSep:  "",
			expected: "2023-01-15 10:30:00 +0000 UTC",
			wantErr:  false,
		},
		{
			name:     "Time pointer with format",
			value:    func() *time.Time { t := time.Date(2023, 1, 15, 0, 0, 0, 0, time.UTC); return &t }(),
			format:   "2006-01-02",
			listSep:  "",
			expected: "2023-01-15",
			wantErr:  false,
		},
		{
			...
			value:    (*time.Time)(nil),
			format:   "",
			expected: "<nil>", // CSV processValue also uses fmt.Sprintf which returns "<nil>" for nil values
			wantErr:  false,
		},
		{
			name:     "Slice with time values and format",
			value:    []interface{}{time.Date(2023, 1, 15, 0, 0, 0, 0, time.UTC), time.Date(2023, 2, 15, 0, 0, 0, 0, time.UTC)},
			format:   "2006-01-02",
			listSep:  ",",
